...
class UploadImage
{
    ...
    public static function upload($file, $folder, $width, $height)
    {
        ...
        if (self::isFile($file) && self::isImage($file)) {
            ...
            // cria a paste temp se ela não existe
    ...
    public static function moveImage($nameImage, $folder, $tempFile, $width, $height)
    {
        ...
        if (file_exists($tempFile) && (filesize($tempFile) > 1) && strlen($tempFile) >= 1 && self::isImage($nameImage)) {
            ...
            if (self::getExtension($tempFile) == "gif") {
                if ($loadImg->getWidth() > $loadImg->getHeight()) {

                    ResizeGif::ResizeToWidth($tempFile, $image['lg']['destiny'], $image["lg"]["width"]);
                    ResizeGif::ResizeToWidth($tempFile, $image['md']['destiny'], $image["md"]["width"]);
                    ResizeGif::ResizeToWidth($tempFile, $image['sm']['destiny'], $image["sm"]["width"]);
                    ResizeGif::ResizeToWidth($tempFile, $image['xs']['destiny'], $image["xs"]["width"]);
                    ResizeGif::ResizeToWidth($tempFile, $image['xxs']['destiny'], $image["xxs"]["width"]);
                } else {

                    ResizeGif::ResizeToHeight($tempFile, $image['lg']['destiny'], $image["lg"]["height"]);
                    ResizeGif::ResizeToHeight($tempFile, $image['md']['destiny'], $image["md"]["height"]);
                    ResizeGif::ResizeToHeight($tempFile, $image['sm']['destiny'], $image["sm"]["height"]);
                    ResizeGif::ResizeToHeight($tempFile, $image['xs']['destiny'], $image["xs"]["height"]);
                    ResizeGif::ResizeToHeight($tempFile, $image['xxs']['destiny'], $image["xxs"]["height"]);
                }
    ...
    private static function createFolderImg($folder)
    {

        if (!file_exists("../public/img/$folder/lg/")) {
            mkdir("../public/img/$folder/lg/");
        }

        if (!file_exists("../public/img/$folder/md/")) {
            mkdir("../public/img/$folder/md/");
        }

        if (!file_exists("../public/img/$folder/sm/")) {
            mkdir("../public/img/$folder/sm/");
        }

        if (!file_exists("../public/img/$folder/xs/")) {
            mkdir("../public/img/$folder/xs/");
        }

        if (!file_exists("../public/img/$folder/xxs/")) {
            mkdir("../public/img/$folder/xxs/");
        }

    }


    private static function createFolderTemp()
    {
        if (!file_exists("./public/img/temp/")) {
            mkdir("../public/img/temp");
        }
    }
    ...
    public static function croppicFinish($tempFile, $folder, $width, $height)
    {
        ...
        $tempFile = str_replace(GetData::getHostMain(), "", $tempFile);
    ...
    public
    static function croppicTemp($file)
    {
        /*
    ...
    */
    ...
    public
    static function croppic($imgUrl, $imgInitW, $imgInitH, $imgW, $imgH, $imgY1, $imgX1, $cropW, $cropH, $angle)
    {
...
//Check write Access to Directory
        ...
        if (!is_writable(dirname($output_filename))) {
            ...
            $response = Array(
                "status" => 'success',
                "url" => GetData::getHostMain() . $output_filename . $type
            );
        }
    ...
    public
    static function uploadBase64($imagem, $folder, $widthMax, $heightMax)
    {
